...
export SPARK_APPLICATION_JAR_LOCATION="/tmp/WordCount.jar"
export SPARK_APPLICATION_MAIN_CLASS="WordCount"
export SPARK_SUBMIT_ARGS=""
export SPARK_APPLICATION_ARGS="/tmp/input/hp.txt /output3"
...
if [ ! -z "${SPARK_APPLICATION_JAR_LOCATION}" ]; then
    ...
    echo "CMD: ${SPARK_HOME}/bin/spark-submit --class ${SPARK_APPLICATION_MAIN_CLASS} --master ${SPARK_MASTER_URL} \
